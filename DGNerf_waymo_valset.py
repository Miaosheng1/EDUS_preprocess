...
""" 整个脚本主要是将 jiaxin 生成的 waymo 点云 (Eval 用的) 转化为 DGNerf 可以使用的 Voxel
    这个代码适用于 ValSet 的生成。
    确保 pose 和 点云处于同一个坐标系， pose 没有被scale 到 [-1,1] 之间
    需要预先 指定的参数如下：
    Dataset: ["KITTI", "Waymo"]
...
DataSet = "eval_waymo"
voxel_type = "drop80_voxel"  ## "voxel"

root_dir = os.path.join("E:\80scene","infer",DataSet)
...
if __name__== "__main__":
    for index in tqdm(range(num_scenes)):
        scene_id = int(seqs[index].split('_')[-2])
        ...
        X_Num = np.round((X_MAX - X_MIN) / Voxel_size).astype(np.int)
