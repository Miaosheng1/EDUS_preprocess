...
""" 整个脚本主要是将 KITTI360 Drop90LeaderBoard 生成的 点云转化为 DGNerf 可以使用的 Voxel
    这个代码适用于 Drop90 的生成。
    注意这里的 Volume Size 发生了变化
...
root_dir = os.path.join("E:\80scene","Drop90")
seqs = os.listdir(root_dir)
num_scenes = len(seqs)

if __name__== "__main__":
    for index in tqdm(range(num_scenes)):
        scene_id = seqs[index].split('_')[1]
        pcd_filename = f"{scene_id}.ply"
        voxel_dir = os.path.join(root_dir,seqs[index])
        ...
        scene_id = "{:04d}".format(int(scene_id))
